$(document).ready(function () {
  var url = base_url + "load-more-brand?page=";
  if (segment_1 == "brand") {
    pagination(url);
  }
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < details_length; i++) {
        ...
        if (details[i].product_name != null) {
          var product_name = details[i].product_name;
        } else {
          var product_name = "";
        }
        if (details[i].product_slug != null) {
          var product_slug = details[i].product_slug;
        } else {
          var product_slug = "";
        }
        if (details[i].product_image != null) {
          var product_image = details[i].product_image;
        } else {
          var product_image = "";
        }
        ...
        if (display == "grid") {
          ...
          html +=
            ...
            '" alt=""></div>';
          html +=
            '<h3><a href="' + product_slug + '">' + product_name + "</a></h3>";
          html += "<div></div>";
          html += "</div>";
        }
      }
