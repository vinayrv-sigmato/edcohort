...
class Cohort extends CI_Controller {
  ...
  function index($id = ''){
    ...
    $data['batch_records'] = $this->common_model->selectWhereorderby('tbl_batch', $where_batch, 'batch_start', 'ASC');
    ...
    if ($_GET) {
      ...
      if (!empty($brandID)) {
        $where .= " and CG.brand_id = " . $brandID . " ";
      }
      if (!empty($product_type)) {
        $where .= " and PR.product_type = " . $product_type . " ";
      }
      if (!empty($board)) {
        $where .= " and CG.board_id = " . $board . " ";
      }
      if (!empty($class)) {
        $where .= " and CG.class_id = " . $class . " ";
      }
      if (!empty($batch)) {
        $where .= " and CG.batch_id = " . $batch . " ";
      }
      if (!empty($course)) {
        $where .= " and CG.product_id = " . $course . " ";
      }
    ...
    $data['group_list'] = $this->cohort_model->cohort_list_limit($where,$limit,$offset,$order);
   // echo $this->db->last_query(); die();
    //print_pre($data['group_list']);
    $order = " CG.date_added DESC";
    $offset = 0;
    $limit = 5;
    $data['ongoing_group_list'] = $this->cohort_model->cohort_list_limit($where,$limit,$offset,$order);
  ...
  function search()
  {
   ...
   // print_ex($data['product_list']);
    redirect(base_url().'cohort?course=' . $data['product_list']['0']->product_id . '&brand='.$brandID.'&product_type='.$product_type.'&board='.$board.'&class='.$class.'&batch='.$batch.'&customer_rating='.$customer_rating.'&date='.$date_posted.'&sort_by='.$sort_by);
  ...
  function create_cohort(){
    ...
    $data['page_head']='Cohort';
    $this->load->view('common/header',$data);
    $this->load->view('cohort/create_cohort',$data);
    $this->load->view('common/footer');

  }

  function create_cohort_submit()
  {
    //print_ex($_POST);
    ...
    $review_tag = $this->input->post('review_tag');
    ...
    $details = $this->common_model->selectWhere('tbl_cohart_group', $where);
    //print_ex($details);
    if (!empty($details)) {
      ...
      $group_id =  $this->common_model->insertData('tbl_cohart_group', $data);
      if($group_id){
        $data = array(
          'group_id' => $product_id,        
        ...
        $group_id =  $this->common_model->insertData('tbl_group_member', $data);
      }

    }
    $this->session->set_flashdata('alert_message','Group Created successfully.');
    redirect($_SERVER['HTTP_REFERER']);
  }

  function check_tag_availability()
  {
    //print_ex($_POST);    
    $review_tag = $this->input->post('review_tag');
    $where = array(
   ...
   // print_ex($details);
    if (!empty($details)) {
      echo '<p style="color:red"> Tag Already Exists</p>'; 
    } else {
      echo '<p style="color:green"> Tag Available </p>';
    }
    //redirect($_SERVER['HTTP_REFERER']);
  }

  function cohort_group(){

    $user_id = $this->session->userdata('user_id');
    $group_id = $this->uri->segment(2);
    $where_board = 'group_id = '.$group_id.' and user_id = '.$user_id.'';
    $group_member_check = $this->common_model->selectWhere('tbl_group_member',$where_board);
    //echo $this->db->last_query();
    if(empty($group_member_check)){
      $data = array(
        'group_id' => $group_id,        
        'user_id' => $user_id,        
        'date_added' => date('Y-m-d')
      ...
      $group_id =  $this->common_model->insertData('tbl_group_member', $data);
      $this->session->set_flashdata('alert_message','You have Joined Group successfully.');
    }

    //print_ex($group_member_check);

    $where_message = "message_status = 1 and MSG.group_id = ".$group_id;
    $data['message_list'] = $this->cohort_model->groupmessage($where_message);

    $wheregroup = " CG.cg_id = ".$group_id."";
    
    //$groupdetails = $this->common_model->selectWhere('tbl_cohart_group', $wheregroup);

    $order = " cg_id DESC";
    $offset = 0;
    $limit = 1;
    
    $data['groupdetails'] = $this->cohort_model->group_list_limit($wheregroup,1,0);

    
    $wheregrplist = 'MBR.user_id = '.$user_id;
    $data['mygroup_list'] = $this->cohort_model->joinedgroup($wheregrplist);

   // echo $this->db->last_query();
    //print_ex($data);
   // die('page');
    $data['page_head']='Cohort Group';
    $this->load->view('common/header',$data);
    $this->load->view('cohort/cohort_group',$data);
    $this->load->view('common/footer');

  }
  function view_cohort_group(){
    ...
    $data['mygroup_list'] = $this->cohort_model->joinedgroup($wheregrplist);
   // echo $this->db->last_query();
    //print_ex($where_message);
   // die('page');
    $data['page_head']='Cohort Group';
    $this->load->view('common/header',$data);
    $this->load->view('cohort/cohort_group_view',$data);
    $this->load->view('common/footer');
  ...
  function leave_group(){
    ...
    $user_id = $this->session->userdata('user_id');
    $group_id = $this->uri->segment(2);
    $where_group = 'group_id = '.$group_id.' and user_id = '.$user_id.'';

    $insert_id =  $this->common_model->deleteData('tbl_group_member', $where_group);
    
    $this->session->set_flashdata('alert_message','Group Leaved successfully.');
    redirect($_SERVER['HTTP_REFERER']);
  ...
  function message_send()
  {
    ...
    $data = array(
      ...
      'message' => $comment,  
      'is_reply' => $is_reply,
      'reply_on_id' => $reply_id,    
      'date_added' => date('Y-m-d')
    );
    $insert_id =  $this->common_model->insertData('tbl_message', $data);
    
    redirect(base_url().'cohort-group/'.$group_id);  
...
function get_cohort_board()
{
  $product_type = 0;
  $brand_id = $this->input->post('brand_id');
  $product_type = $this->input->post('product_type');
  if ($brand_id) {
    echo $this->cohort_model->fetch_cohort_board_list($brand_id,$product_type);
  }
}

function get_cohort_class() {
 ...
 echo $this->cohort_model->fetch_cohort_class_list($brand_id,$product_type,$board_id);
 // echo $output;
}

function get_cohort_batch()
{
  $product_type = 0;
  //$board_id = $this->input->post('board_id');
  $product_type = $this->input->post('product_type');
  ...
  $class_id = $this->input->post('class_id');
  if ($board_id) {
    echo $this->cohort_model->fetch_cohort_batch_list($brand_id,$product_type,$board_id,$class_id);
  }
}
