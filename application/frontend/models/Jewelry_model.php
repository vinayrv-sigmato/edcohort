<?php
class Jewelry_model extends CI_Model {
  ...
  function jewelry_list($where,$order)
  { 
      if($where!=""){        
        $where="WHERE ".$where;
      }      
  ...
  function getProductReview($where,$order='')
  {
      if($where!=""){        
        $where="WHERE ".$where;
      }
      if($order!=""){
        ...
        $order_query="ORDER BY product_review_added DESC ";
      }
      $query=$this->db->query("SELECT pr.*,c.firstname,c.lastname,p.product_name,p.product_slug FROM tbl_product_review as pr 
        join tbl_product as p ON pr.product_id=p.product_id 
