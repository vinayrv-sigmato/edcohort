...
class brands_model extends CI_Model {
	...
	function brands_list($where)
	{  
	  	if($where!=""){
	    	$where="WHERE ".$where;
	  	}
	  	$query = $this->db->query("select *  from tbl_brand as B ".$where." ");	  
		return $query->result();
	}
	function brands_count($where)
	{      
	  if($where!=""){
	    $where="WHERE ".$where;        
	  }
	  $query = $this->db->query("select count(brand_id) as brand_count from tbl_brand as B ".$where." ");
	  return $query->result();
	}     	    
	function brands_limit($where,$limit='',$offset=0)
	{  
	  	if($where!=""){
	    	$where="WHERE ".$where;
	  	}
	  	$query = $this->db->query("select *  from tbl_brand B ".$where." LIMIT ".$offset." , ".$limit." ");	  
		return $query->result();
	} 
