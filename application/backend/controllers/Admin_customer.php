...
class admin_customer extends CI_Controller
{
    ...
    function add_customer_submit()
    {
        ...
        if (count(array_filter($result))) {
            $this->session->set_flashdata('alert', 'User Exist!');
    ...
    { 
           ...
           $validation_response =  $this->admin_model->edit_validation($customer_id,$email);
           if($validation_response)
           {
            $this->session->set_flashdata('alert','Email ID all ready exist!');
            //redirect(base_url().'admin_customer');
