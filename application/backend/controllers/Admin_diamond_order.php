...
class admin_diamond_order extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->load->model('diamond_order_model');
    ...
    function index()
    {
        ...
        $this->load->view('diamond_order/diamond_order_list_view',$data);
    ...
    function loadData()
    {
        ...
        $filter_stock_id=$this->input->get('filter_stock_id');
        ...
        $where ='stock_id > 0  AND status != "idle" ';
        if($group_id!='1'){
            $where .=" AND vendor_id ='".$user_id."'";
        }
        if($filter_stock_id!="")
        {
            $where .=' AND stock_id = "'.$filter_stock_id.'"';
        }
        if($filter_vendor!="")
        {
            $where .=' AND vendor_id = "'.$filter_vendor.'"';
        }
        ...
        $order_by='ORDER BY stock_id';
        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        $diamond_order_total=$this->diamond_order_model->orderTotal($where,$order_by);
        $data['diamond_order_count'] =$diamond_order_total['0']->diamond_order_count;
        
        $per_page= ($per_page) ? $per_page : 100 ;
        $config['base_url'] = base_url().'admin_diamond_order/loadData';
        $config["total_rows"] = $data['diamond_order_count'];
        ...
        $records=$this->diamond_order_model->diamond_order_list($per_page,$page,$where,$order_by);

        echo json_encode(array('records'=>$records,'page_link'=>$page_link,'total_records'=>$data['diamond_order_count']));      
    }
