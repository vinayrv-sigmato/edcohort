$(document).ready(function(){ 
    var url=base_url+'admin_order/loadData?page='; 
    pagination(url);
...
function submitForm() {
    ...
    var set_url=base_url+'admin_order?'+value_form;
        ...
        $.ajax({
            ...
            success: function(data){ 
               ...
               for(i=0;i<details_length;i++) {   
                    ...
                    html +='<tr id="tr_'+details[i].order_id+'">';
                    html +='            <td><input type="checkbox" class="multi-chk-complete"  id="basic_'+details[i].order_id+'" name="check" value="'+details[i].order_id+'" />   ';
                    html +='                <label for="basic_'+details[i].order_id+'"></label></td>   ';
                    html +='            <td><strong>'+details[i].order_reference+'</strong></td>';
                    html +='            <td>'+details[i].firstname+' '+details[i].lastname+'</td>       ';
                    html +='            <td><strong id="status_'+details[i].order_id+'">'+details[i].order_status+'</strong></td>';
                    html +='            <td>'+details[i].total+'</td>';
...
function setAction()
{
    ...
    else 
    {
        alertify.confirm(
            ...
            {                             
                $.ajax({
                    url: base_url+'admin_order/action',
                    dataType: 'json',
                    type: 'post',            
