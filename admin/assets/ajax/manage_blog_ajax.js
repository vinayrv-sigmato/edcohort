$(document).ready(function(){ 
    var url=base_url+'admin_blog/loadData?page='; 
    pagination(url);
...
function submitForm() {
    ...
    var set_url=base_url+'admin_blog?'+value_form;
            ...
            success: function(data){ 
               ...
               for(i=0;i<details_length;i++) { 
                ...
                if(details[i].date_edited!=null){ date_edited=moment(details[i].date_edited).format('MMM Do YYYY'); }else{ date_edited=''; }               
                    html +='<tr id="tr_'+details[i].blog_id+'">';
                    html +='            <td><input type="checkbox" class="multi-chk-complete"  id="basic_'+details[i].blog_id+'" name="check" value="'+details[i].blog_id+'" />';   
                    html +='                <label for="basic_'+details[i].blog_id+'"></label></td>';   
                    html +='            <td><strong>'+details[i].blog_title+'</strong></td>';
                    html +='            <td>'+details[i].blog_dec+'</td>';                                     
                    html +='            <td><strong id="status_'+details[i].blog_id+'">'+status+'</strong></td>';                    
    ...
    else 
    {
       // alert();
        alertify.confirm(
            ...
            {                             
                $.ajax({
                    url: base_url+'admin_blog/action',
                    dataType: 'json',
                    type: 'post',            
