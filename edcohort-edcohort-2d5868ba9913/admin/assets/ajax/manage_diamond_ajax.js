$(document).ready(function(){
    var url=base_url+'admin_diamond/load_more_data?page='; 
    pagination(url);
...
function submitForm() {
    ...
    var set_url=base_url+'admin_diamond?'+value_form+'&'+$.param({ "shape": checkbox_str,"certificate": cert_str});
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for(i=0;i<details_length;i++) {
                    ...
                    if(details[i].vendor_name != null){ var vendor_name=details[i].vendor_name; }else{ var vendor_name=""; }
                    ...
                    if(details[i].cost_carat != null){ var cost_carat=parseFloat(details[i].cost_carat).toFixed(2); }else{ var cost_carat=""; }
                    if(details[i].availability != null){ var availability=details[i].availability; }else{ var availability=""; }
                    if(details[i].new_discount != null){ var new_discount=parseFloat(details[i].new_discount).toFixed(2);; }else{ var new_discount=""; }
                    if(details[i].report_no != null){ var report_no=details[i].report_no; }else{ var report_no=""; }
                    if(details[i].diamond_status != null){ var diamond_status=details[i].diamond_status; }else{ var diamond_status=""; }

           
                    html +='<tr style="font-size:12px; font-weight:bold;" id="tr_'+details[i].diamond_id+'">';
                    html +='    <td>';  
                    html +='      <a href="'+base_url+'admin_diamond/diamond_details/'+details[i].diamond_id+'">';                                     
                    html +='        <i class="fa fa-search-plus"  title="View details" data-toggle="tooltip" data-placement="right"></i>';
                    html +='      </a>';

                    html +='      <a href="'+base_url+'admin_diamond/edit_diamond_details/'+details[i].diamond_id+'">';                                     
                    html +='        <i class="fa fa-pencil"  title="Edit details" data-toggle="tooltip" data-placement="right"></i>';
                    html +='      </a>';
                    html +='    </td>';
                  ...
                  html +='    <td> </td>';
                    ...
                    html +='    <td><strong id="status_'+details[i].diamond_id+'">'+diamond_status+'</strong></td>';
...
function delete_data()
{
     //alert('delete');
      ...
      {     
            ...
            {
                    $.ajax({
                        url: base_url+'admin_diamond/delete_diamond',
                        dataType: 'json',
                        type: 'get',            
...
function setAction()
{
    ...
    {
        alert_boot("Please Select Diamond!");
    }
    else if(action=="")
    {
    ...
    else 
    {
        alertify.confirm(
            ...
            {                             
                $.ajax({
                    url: base_url+'admin_diamond/diamond_action',
                    dataType: 'json',
                    type: 'post',            
                    data: { "diamond_id": checkboxArr, action: action},                                         
                    success: function(data){
 ...
 function print_data()
     ...
     //alert('print data');
        ...
        if(checkbox_arr.length)
        {
            ...
            $.ajax({
            url: base_url+'admin_diamond/print_data',
            dataType: 'json',
            type: 'get',            
            data: { "all_id": checkbox_arr },            
            success: function(data){
                    ...
                    for(i=0;i<data_length;i++)
                    {
                        ...
                        html +='    <td>'+table_d+'</td>';
                         if(data[i].new_discount != null){ var new_discount=parseFloat(data[i].new_discount).toFixed(1); }else{ var new_discount=""; }
                        html +='    <td>'+new_discount+'</td>';
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   {    
        ...
        if(checkbox_arr.length)
        {
            //alert(checkbox_arr.length);
            $.ajax({
            url: base_url+'admin_diamond/download_file',
            dataType: 'json',
            type: 'get',            
            data: { "all_id": checkbox_arr },            
            success: function(data){
...
function export_list()
{
  ...
  if(checkbox_arr.length)
  {
      location.href=base_url+'admin_diamond/export_diamond?all_id='+checkbox_arr    
  }
  else
  {
      if(confirm('Do You Want To Download All Record ?'))
      {
          location.href=base_url+'admin_diamond/export_all_diamond?all_id='
      }
