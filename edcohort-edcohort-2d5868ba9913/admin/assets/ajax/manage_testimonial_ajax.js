$(document).ready(function(){ 
    var url=base_url+'admin_testimonial/loadData?page='; 
    pagination(url);
...
function submitForm() {
    ...
    var set_url=base_url+'admin_testimonial?'+value_form;
            ...
            success: function(data){ 
               ...
               for(i=0;i<details_length;i++) { 
                ...
                if(details[i].date_edited!=null){ date_edited=moment(details[i].date_edited).format('MMM Do YYYY'); }else{ date_edited=''; }               
                    html +='<tr id="tr_'+details[i].testimonial_id+'">';
                    html +='            <td><input type="checkbox" class="multi-chk-complete"  id="basic_'+details[i].testimonial_id+'" name="check" value="'+details[i].testimonial_id+'" />';   
                    html +='                <label for="basic_'+details[i].testimonial_id+'"></label></td>';   
                    html +='            <td><strong>'+details[i].testimonial_by+'</strong></td>';
                    html +='            <td>'+details[i].testimonial+'</td>';                                     
                    html +='            <td>'+status+'</td>';                    
    ...
    else 
    {
       // alert();
        alertify.confirm(
            ...
            {                             
                $.ajax({
                    url: base_url+'admin_testimonial/action',
                    dataType: 'json',
                    type: 'post',            
