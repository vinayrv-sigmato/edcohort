$(document).ready(function(){  
    var url=base_url+'load-ring-builder?page='; 

    if(segment_1=="ring-builder"){
        pagination(url);
    }      
    //alert(base_url); 
            ...
            success: function(data){                
                ...
                {           
                    ...
                    if(display=="grid")
                    {
                        ...
                        html +='<div class="col-md-3 col-sm-6 col-xs-12 product-col product-space item position-relative product-overlay">';
                        html +='  <div class="product-div product-bg position-relative product-overlay">';
                        html +=' <a href="'+base_url+'ring-builder-details/'+details[i].product_slug+'">';                        
                        ...
                        // html +='        <a href="javascript:void(0)" onClick="add_wish('+details[i].product_id+',this,0)" class="highlight-button-dark btn btn-small no-margin-right quick-buy-btn radius-flat" title="Add to Wishlist" data-wow-duration="300ms"><i class="fa fa-heart-o"></i></a>';
                        // html +='        <a href="javascript:void(0)"  onClick="add_compare('+details[i].product_id+',this,0)" class="'+text_color+' btn btn-small no-margin-right quick-buy-btn radius-flat" title="'+compare_title+'" data-wow-duration="600ms"><i class="fa fa-exchange"></i></a>';
                        // }
                        // html +='        <a href="'+base_url+'jewelry-details/'+details[i].product_slug+'" class="highlight-button-dark btn btn-small no-margin-right quick-buy-btn radius-flat" title="View Details" data-wow-duration="900ms"><i class="fa fa-eye"></i></a>'; 
                        // html +='      </div>';
                        // html +='    </div>';
                        html +='  </div>';
                        html +='  <div class="card-body">';
                        html +='    <h4 class="card-title fixed-height"> <a href="'+base_url+'ring-builder-details/'+details[i].product_slug+'">'+product_name+'</a> </h4>';
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function choose_setting(jewelry_id,e,grid)
{  
    validate=1;
    var form = document.getElementById("cart_form").elements;
    for(var i = 0; i < form.length; i++){
          ...
          $.ajax({
            ...
            success: function(data){
                if(data.status)
                {
                    ...
                    {
                        window.location.href = base_url+"ring-builder-diamond";
                    }
                    //window.location.href = base_url+"ring-builder-diamond";
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function ringToBag(jewelry_id,e,grid)
{
    var cart_quantity=$("#cart_quantity").val();        
    var value_form = $('#cart_form').serialize()+'&'+$.param({ "jewelry_id": jewelry_id, "quantity": cart_quantity });

    $.ajax({
        url: base_url+'ring-to-bag',
        dataType: 'json',
        type: 'get',            
        data: value_form,                                         
        success: function(data){
