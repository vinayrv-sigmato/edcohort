...
class Design extends CI_Controller {
	...
	public function __construct()
  {
	...
	{
      $where="design_id > 0 AND type = '' OR type IS NULL";
      ...
      $data['where'] = $where; 		
			...
			$this->load->view('design/design_list',$data);
	...
	function design_details()
  {
    ...
    //  print_ex($where);  
      ...
      //print_ex($records);
       ...
       if($records[0]->type == 'custom'){
          $id = $records[0]->product_id;
          $where1=array('id'=>$id,'type'=>'product');
          $image_array=$this->admin_model->selectWhere('tbl_image',$where1);
        
        }else{
          $where1=array('design_id'=>$design_id);
          $image_array=$this->admin_model->selectWhere('tbl_custom_design_image',$where1);
        }

      $where2=array('design_id'=>$design_id);
      $process=$this->admin_model->selectWhere('tbl_design_process',$where2);
  ...
  function design_edit()
  {
     ...
     // $process=$this->design_model->selectJoin('tbl_design_process','process_id','tbl_process_image','process_id','design_id',$design_id);
      ...
      $this->load->view('common/header');
      $this->load->view('common/sidebar',$data);
      $this->load->view('design/design_edit',$data);
      $this->load->view('common/footer');
  }

  function design_edit_submit()
  {
    $ad_user_id=$this->session->userdata('jw_admin_id');
    ...
    if($this->form_validation->run() == FALSE)
    {
      ...
      redirect(base_url().'design/design_edit/'.$design_id);
    }
      ...
      if(!empty($processing_status))
       ...
       if(!empty($ad_user_id))
        ...
        $this->admin_model->updateData('tbl_design',$data,array('design_id'=>$design_id));
      ...
      $insert_id=$this->admin_model->insertData('tbl_design_process',$data);
          ...
          foreach ($product_image_array as $key => $value) 
            {
               $data_image=array(
                ...
                $this->admin_model->insertData('tbl_process_image',$data_image);
            } 
  ...
  function delete()
  {
    ...
    $this->admin_model->deleteData('tbl_design',$where);
