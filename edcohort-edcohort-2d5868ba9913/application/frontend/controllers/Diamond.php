  ...
  class Diamond extends CI_Controller {
    ...
    { 
      ...
      $this->load->view('diamond/diamond',$data);
    ...
    function load_more_data()
    {
      ...
      if (trim($carat_from) != '' && trim($carat_to)) {      
        ...
        $search .= " Carat : From ('$carat_from') To ('$carat_to')";
      }
      ...
      $where .= $this->strRepBetween('m_length', 'm_length');
      $where .= $this->strRepBetween('m_depth', 'm_depth');
      $where .= $this->strRepBetween('m_width', 'm_width');  
      ...
      {        
        ...
        $search .= " AND weight BETWEEN ('".$split1."') AND ('".$split2."')";     
      }     
      // if(!empty($user_id)) 
      // {
        ...
        if(!empty($total))
        {
         ...
         $search .= " Price From (".$splittotal1.") To (".$splittotal2.")";
       }
   //  }
     ...
     foreach($pointer as $rows){
      ...
      if($i == 1){
       if($pointer_count == 1){
        ...
        $search .= " AND ".$open." weight BETWEEN  ".$min."  AND  ".$max." ";
      }
      if($pointer_count >=2){
        $where .= "  AND ".$open." weight BETWEEN  ".$min."  AND  ".$max."  " ;
        $search .= " AND ".$open." weight BETWEEN  ".$min."  AND  ".$max." ";       
      ...
      $search .= " OR weight BETWEEN  ".$min." AND ".$max." "; 
    }  
    $i++;
  }
  $where .= "".$close."";    
       ...
       //$search .= " AND ".$open." weight BETWEEN  ".$min."  AND  ".$max." ";
      // $search .= " OR weight BETWEEN  ".$min." AND ".$max." ".$close." "; 

$search .= $this->strRepIn('Shape', 'checkbox');  
$search .= $this->strRepIn('Cut', 'cut');  
$search .= $this->strRepIn('Polish', 'polish');  
$search .= $this->strRepIn('Symmetry', 'symmetry');  
$search .= $this->strRepIn('Clarity', 'clarity');  
$search .= $this->strRepIn('Fluorescence', 'fluorescence');
$search .= $this->strRepIn('Lab', 'cert');       
$search .= $this->strRepIn('Culet', 'culet'); 
$search .= $this->strRepBetween('Table per', 'table_per');
$search .= $this->strRepBetween('Depth', 'depth_per');
$search .= $this->strRepBetween('Crown Angle', 'cr_angle');
$search .= $this->strRepBetween('Crown Ht', 'cr_height');
$search .= $this->strRepBetween('Pavillion Angle', 'pav_angle');
$search .= $this->strRepBetween('Pavillion Depth', 'pav_height');
$search .= $this->strRepBetween('Girdle Perct', 'girdle_per');
$search .= $this->strRepBetween('Star len', 'star_len');
$search .= $this->strRepBetween('M length', 'm_length');
$search .= $this->strRepBetween('M depth', 'm_depth');
$search .= $this->strRepBetween('M width', 'm_width');
      //  print_ex($where);
$records_count = $this->diamond_model->diamond_count($where);
...
if(!empty($user_id)){
  ...
  $data_details=array(
    'user_id' => $user_id,                     
    'search' => $search,      
    'search_count' => $data['records_count'],               
    'date_added'=> date('Y-m-d H:i:s'),      
  );
  $this->common_model->insertData('tbl_search_history',$data_details); 
} 
...
function diamond_price($vendor_id,$weight,$rapnet,$rapnet_discount)
{
  ...
  return $data;
        // currency tbl_cureency se value 
}
function diamond_details()
{
  ...
  $name =  number_format($records['0']->weight,2).' Carat '. $records['0']->shape_full. ' Shape '. $records['0']->color.' Color '. $records['0']->grade.' Clarity '. $records['0']->cut_full.' Cut '. $records['0']->polish_full.' Polish '. $records['0']->symmetry_full.' Symmetry '. $records['0']->fluor_full.' Fluorescence';       
  if(!empty($user_id)){

    $data_details=array(
      'user_id' => $user_id, 
      'item_id' => $records['0']->stock_id,
      'type' => 'diamond',  
      'title' => $name,                    
      'date_added'=> date('Y-m-d H:i:s'),      
    );
...
function dna_details()
{
  ...
  echo json_encode($data);    
       ...
       // $this->load->view('common/header',$data);
      // $this->load->view('diamond/diamond_dna_details',$data);
     //   $this->load->view('common/footer');
}
...
{        
  ...
  $this->excel->getActiveSheet()->setCellValueByColumnAndRow('21','1' ,'Origin'); 
  $this->excel->getActiveSheet()->setCellValueByColumnAndRow('22','1' ,'Country'); 
  ...
  {  
   ...
   $this->excel->getActiveSheet()->setCellValueByColumnAndRow('14',$i,'&#3647'.round($row->currency));        
   ...
   $this->excel->getActiveSheet()->setCellValueByColumnAndRow('21',$i,$row->origin);    
   $this->excel->getActiveSheet()->setCellValueByColumnAndRow('22',$i,$row->country);      
...
function print_details()
{
  ...
  foreach ($records as $row) {
    ...
    $row->rapnet_discount = $price['new_discount'];
    $row->cost_carat = $price['new_rapnet'];
    $row->total_price = $price['new_totalprice'];
    $row->currency = $price['currency'];
  }
...
$html = $this->load->view('diamond/GeneratePdfView', $data, true);
...
{ 
  ...
  <th style="border: 1px solid black; text-align: center;" >Thai Baht</th>                
  ...
  foreach($records as $row) 
  {
    ...
    <td style="border: 1px solid black; text-align: center;">&#3647;'.round($row->currency).'</td>
 ...
 { 
  ...
  $this->load->view('diamond/compare_diamond');
  $this->load->view('common/footer');
}  
