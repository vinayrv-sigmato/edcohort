...
class Ring_builder_diamond extends CI_Controller {
  ...
  { 
      ...
      //print_ex($data['ring_setting_diamond_shape']);
      //edited by arti start
    //   if(empty($data['ring_setting_diamond_shape'])){
    //     redirect(base_url().'ring-builder');
    //   }
      //edited by arti end  
      $this->load->view('common/header',$data);
      $this->load->view('ring_builder/diamond',$data);
  ...
  function load_more_data()
  {
    if(!$this->input->is_ajax_request()){ exit('No direct script access allowed'); }
      ...
      $ring_setting_diamond_shape = $this->session->userdata('ring_setting_diamond_shape');


      $page=$this->input->get('page');
      $per_page=$this->input->get('per_page');
     
      $where="diamond_id > 0 AND total_price > 100";    
      ...
      if(count($ring_setting_diamond_shape))
      {
          $q1=implode("','",$ring_setting_diamond_shape);
          $where .= " AND shape_filter IN ('".$q1."')";
      }   
      ...
      $config['base_url'] = base_url().'load-ring-builder-diamond';
  ...
  function diamond_details()
  {
      ...
      $data['page_head'] = 'Diamond Details';     
        $data['ring_setting_diamond_data'] = $this->session->userdata('ring_setting_diamond_data');
      $this->load->view('common/header',$data);
      $this->load->view('ring_builder/diamond_details',$data);
      $this->load->view('common/footer');
  }
  ...
  {     
    ...
    if(count($records))
    {
      if($user_id!="")
      {
          $where=array('product_id'=>$diamond_id,'product_type'=>'diamond','created_by'=>$user_id,);
          ...
          $where1=array('created_by'=>$user_id);
  ...
  {     
    ...
    if(count($records))
    {
      ...
      // if($user_id!="")
      // {
          //$where=array('product_id'=>$stock_id,'product_type'=>'diamond','created_by'=>$user_id,);
          //$cart_detail=$this->common_model->selectWhere('tbl_cart',$where);
          // if(!count($cart_detail))
          // {        
            $data=array(
              ...
              'price'=>$records['0']->total_price,
              'total_price'=>$records['0']->total_price,
              'name'=>$records['0']-> weight.' Carat '.$records['0']-> shape.' diamond',
              'description'=>$records['0']-> weight.' Carat '.$records['0']->shape.', '.$records['0']->cut_full.'-cut, '.$records['0']->color.'-color, '.$records['0']->grade.'-clarity diamond',
              'image'=>$image_show,
              'shape_name'=>$records['0']-> shape_full,
              //'created_at'=>date('Y-m-d H:i:s'),
              //'created_by'=>$user_id,
            );
            //$cart_id=$this->common_model->insertData('tbl_cart',$data);
          ...
          //$where1=array('created_by'=>$user_id);
          //$cart_details=$this->cart_model->get_cart($where1);  
          echo json_encode(array('records'=>$data,'message'=>'ok'));

          $this->session->set_userdata('ring_setting_diamond_data',$data);
