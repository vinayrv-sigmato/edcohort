...
class Complaint extends CI_Controller {
  ...
  function index($id = '')
  {
	 ...
	 // print_ex($brand_id);
	  ...
	  $where_prodcut=" product_status = 'active' and product_brand = ".$brandID."";
	...
	//print_ex($date['product_list']);
	  ...
	  if(!empty($course)){
	  $where_complaint = 'PR.status = "active" and product_id = '.$course.'';
	  $data['complaint_records'] = $this->complaint_model->getcomplaint($where_complaint);
	 
	 
	  $data['count_total_rating'] = $this->complaint_model->count_total_rating($course);
	  
	  $data['rating_total'] = $this->complaint_model->rating_total($course);	  
	  $data['five_total'] = $this->complaint_model->rating_total($course,5);
	  $data['four_total'] = $this->complaint_model->rating_total($course,4);
	  $data['three_total'] = $this->complaint_model->rating_total($course,3);
	  $data['two_total'] = $this->complaint_model->rating_total($course,2);
	  $data['one_total'] = $this->complaint_model->rating_total($course,1);
	  
	  $data['complaint_search_count'] = $this->complaint_model->complaint_search_count($course);
	  ...
	  $where_complaint_search = 'user_id != "" and product_id = '.$course.'';
	  $data['complaint_search'] = $this->common_model->selectWhereorderby('tbl_product_complaint_search',$where_complaint_search,'prs_id','ASC');
	
	  
	  }
	
		//print_ex($data['complaint_search']);
		if($course){
		 $dataInser = array(
            ...
            $insert_id = $this->common_model->insertData('tbl_product_complaint_search', $dataInser);
		}
  ...
  {  
      ...
      $complaint = $this->jewelry_model->getProductcomplaintLimit($where,10,0);

      $where = "product_id = ".$jewelry_id." and status='active' ";
      $complaint_count = $this->jewelry_model->getProductcomplaintCount($where);      

      //$where = "product_id = ".$jewelry_id."" ;
      $complaint_sum = $this->jewelry_model->getProductcomplaintSum($where);
  ...
  function load_product_complaint()
  {
      ...
      $records_count = $this->jewelry_model->getProductcomplaintCount($where);      
      ...
      $records = $this->jewelry_model->getProductcomplaintLimit($where,$per_page,$page);
