...
class Ring_builder extends CI_Controller {
  ...
  function index()
  { 
    $data['ring_setting_diamond_data'] = $this->session->userdata('ring_setting_diamond_data');
    $data['ring_setting_diamond_shape'] = $this->session->userdata('ring_setting_diamond_shape');

    //print_ex($data['ring_setting_diamond_data']);
      ...
      $this->session->set_userdata('builder_category_id',$parent_id);      
      ...
      //$data['metal_color']=$this->jewelry_model->getProductFilter("name LIKE '%Metal Color%'");
      //print_ex($data);
      
      $this->load->view('common/header',$data);
      $this->load->view('ring_builder/ring_builder',$data);
  ...
  private function get_child_category($category_id,$block="") 
  {
      ...
      //$query = $this->db->query(" SELECT category_id FROM tbl_category c where c.parent_category = '11'");
      $result = $query->result();
  ...
  function load_more_data()
  {
      ...
      $CAT_ID = $this->session->userdata('builder_category_id');

      $ring_setting_diamond_data = $this->session->userdata('ring_setting_diamond_data');
      $bu_diamond_shape = $ring_setting_diamond_data['shape_name'];
      //print_ex($ring_setting_diamond_data);
      ...
      $where .=" AND product_is_builder = '1'";
      if($segment_2=='sale'){
      ...
      if(count($bu_diamond_shape))
      {            
          $where .= " AND diamond_shape = '".$bu_diamond_shape."'";         
      }
      ...
      {   
        ...
        $order .= "product_builder_sort ASC ";
      }

      $where=$where.$whereOr;
      //print_ex($where.$whereOr);
      ...
      $config['base_url'] = base_url().'load-ring-builder';
      ...
      foreach ($records as $row) 
      {
        ...
        if(!$product_price_arr['price_var'] && $product_price_arr['price']){
          $row->product_price_show='$'.$product_price_arr['price'];
            ...
            $where="product_id='".$row->product_id."' and is_active='1' and variation_attributes_name='Finish Level' and variation_attributes_value='Semi Mount' ";
  ...
  {  
      //print_pre($this->session->userdata('ring_setting_diamond_shape'));
      //print_ex($this->session->userdata('ring_setting_data'));
      ...
      if(!empty($records)){
        ...
        //print_ex($data['product_variation']);
        /*foreach ($data['product_variation'] as $varkey => $varvalue){
          ...
          $query_var_attr = $this->db->query("select count(variation_id) as count from tbl_product_price_variation_attributes where variation_id = '".$varvalue->variation_id."' and variation_attributes_name = 'Finish Level' and variation_attributes_value = 'Semi Mount' ");
          $check_var_attr = $query_var_attr->result();
          if($check_var_attr['0']->count){            
...
/*        $html .='  <select name="'.$n_name.'" id="'.$n_name.'" class="form-control cart-form-control" required="" onchange="variation()">'; 
        //$html .='    <option value="">Select an Option</option>';
                  foreach ($data['product_option'] as $key => $value){
                    ...
                    if(in_array($optvalue->name, $o_array) && in_array($value->value, $v_array)){
                      if($optvalue->name=='Finish Level'){
                        if($value->value=='Semi Mount'){
        ...
        foreach ($product_video_link as $row) {
          if($row->product_video !='' && checkLink($row->product_video)=='200'){            
        ...
        if($records['0']->product_global_addons=='active'){
          ...
          $pro_cat=$this->jewelry_model->pro_cat("product_id = ".$jewelry_id);
          //print_ex($pro_cat);
        ...
        {          
          $where1 = ' price_filter > 0 AND product_status="active" and product_is_builder = "1" AND category_id IN ("'.$records['0']->category_id.'")';
        } 
      ...
      // $d=json_decode($a['0']->variation_attributes);
       //print_ex($data['product_feature']);
      $data['page_head'] = 'Jewelry Details';
      
        $data['ring_setting_diamond_data'] = $this->session->userdata('ring_setting_diamond_data');
        
      $this->load->view('common/header',$data);
      $this->load->view('ring_builder/jewelry_details',$data);
      $this->load->view('common/footer');
  }  
  function ring_review()
  {  
      //print_pre($this->session->userdata('ring_setting_diamond_shape'));
      //print_pre($this->session->userdata('ring_setting_data'));
      //print_ex($this->session->userdata('ring_setting_diamond_data'));
      $ring_setting_data=$this->session->userdata('ring_setting_data');
      $ring_setting_diamond_data=$this->session->userdata('ring_setting_diamond_data');

      //$name=$ring_setting_data['name'].' with '.$ring_setting_diamond_data['name'];
      //$description=$ring_setting_data['name'].' is set with '.$ring_setting_diamond_data['description'];
      //$price=round($ring_setting_data['total_price']+$ring_setting_diamond_data['total_price']);
      //print_pre($ring_setting_data);
      //print_ex($ring_setting_diamond_data);
      ...
      $where = "product_sku = '".$ring_setting_data['stock_id']."'";
      //$where = "product_slug = '".$segment_2."'";
      $records= $this->jewelry_model->jewelry_list($where); 
      if(empty($records)){
        redirect(base_url().'ring-builder');
      }
     ...
     if(count($records)){
        ...
        // $video='ftp_upload/jewelry/video/'.$records['0']->video_1;
        // if(file_exists($video) && $records['0']->video_1)
        // {
        //     $video_array[]=$video;
        // }
        // $certificate='ftp_upload/jewelry/certificate/'.$records['0']->certificate;
      ...
      $data= array(
          ...
          'description'=>$ring_setting_data['name'].' is set with '.$ring_setting_diamond_data['description'],
          'price'=>round($ring_setting_data['total_price']+$ring_setting_diamond_data['total_price']),
          'price_setting'=>round($ring_setting_data['total_price']),
          'price_diamond'=>round($ring_setting_diamond_data['total_price']),
          'attributes'=>$ring_setting_data['attributes'],
      );
      ...
      $this->load->view('common/header',$data);
      $this->load->view('ring_builder/ring_review',$data);
      $this->load->view('common/footer');
  }
  ...
  function add_cart()
  {
      ...
      if($user_id!="")
      {
          $product_sku=$ring_setting_data['stock_id'];
          $where = "product_sku = '".$product_sku."'";       
          $records= $this->jewelry_model->jewelry_list($where); 

          if(count($records))
          { 
              $ring_setting_data['is_ring_builder'] = '1';           
              $ring_setting_data['created_at'] = date('Y-m-d H:i:s');           
              $ring_setting_data['customer_id'] = $user_id;
              $cart_id=$this->common_model->insertData('tbl_cart',$ring_setting_data);
          } 

          $stock_id=$ring_setting_diamond_data['stock_id'];
          $where = 'stock_id = '."'".$stock_id."'";      
          $records = $this->diamond_model->diamond_list($where);
       
          if(count($records))
          {        
              $ring_setting_diamond_data['cart_id'] = $cart_id;           
              $ring_setting_diamond_data['created_at'] = date('Y-m-d H:i:s');           
              $ring_setting_diamond_data['customer_id'] = $user_id;
              $cart_id=$this->common_model->insertData('tbl_cart_diamond',$ring_setting_diamond_data);
          } 
          $where=array('customer_id'=>$user_id);
          $cart_details=$this->cart_model->get_cart($where);  
          ...
          if($get_cookie!="")
          {
              ...
              { 
                  $ring_setting_data['is_ring_builder'] = '1';           
                  $ring_setting_data['created_at'] = date('Y-m-d H:i:s');           
                  $ring_setting_data['cookie_id'] = $get_cookie;
                  $cart_id=$this->common_model->insertData('tbl_cart_cookie',$ring_setting_data);
              } 
              ...
              {        
                  ...
                  $ring_setting_diamond_data['created_at'] = date('Y-m-d H:i:s');           
                  $ring_setting_data['cookie_id'] = $get_cookie;
                  $cart_id=$this->common_model->insertData('tbl_cart_cookie_diamond',$ring_setting_diamond_data);
              }
          }
          else
          {
              $get_cookie=mt_rand().':'.$_SERVER['SERVER_ADDR'];
          ...
          $this->session->unset_userdata('ring_setting_data');
          $this->session->unset_userdata('ring_setting_diamond_data');
          $this->session->unset_userdata('ring_setting_diamond_shape');
      ...
      // {
          ...
          // }
            ...
            if($status){
              ...
              $product_diamond_shape=$this->common_model->selectOne('tbl_product_diamond_shape','product_id',$jewelry_id);
              foreach ($product_diamond_shape as $key => $value) {
                  $diamond_shape_array[]=$value->diamond_shape;
              }
              $diamond_shape_array=array_unique($diamond_shape_array);
                
              $this->session->set_userdata('ring_setting_diamond_shape',$diamond_shape_array);
              $this->session->set_userdata('ring_setting_data',$data);
            }            
          // }else{
  ...
  function load_product_review()
  {
      ...
      $records_count = $this->jewelry_model->getProductReviewCount($where);      
      ...
      $records = $this->jewelry_model->getProductReviewLimit($where,$per_page,$page);
  ...
  public function header_search()
    {  
        ...
        if($keyword)
        {
         ...
         // echo  $where;
          $data = $this->jewelry_model->jewelry_list_limit($where,'50','1');   
         // print_ex($data);   
        }
        echo json_encode($data);       
    }
