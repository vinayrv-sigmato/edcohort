...
class Ranking extends CI_Controller {
  ...
  { 
   ...
   // print_ex($brand_id);
    ...
    $where_prodcut=" c_status = 1 and c_date >= CURDATE() and c.status = 1 and c.customer_type = 1";
      
    if(!empty($getclass)){
      $where_prodcut .=" and pr.class_id = ".$getclass." ";
      
    }
  	$getbrandid = $this->input->get('brand');
  	if(!empty($getbrandid)){
        $where_prodcut .=" and pr.brand_id = ".$getbrandid." ";
        
      }

      $getbrandid = $this->input->get('brand');
    if(!empty($getbrandid)){
        $where_prodcut .=" and c.brand_id = ".$getbrandid." ";
        
      }
      
      
      $order = "c_date ASC";
      $data['ranking_list'] = $this->ranking_model->getProductranking($where_prodcut,$order);
  	
  	
  	 $getclass = $this->input->get('class');


      $where_prodcut=" c_status = 1 and c_date >= CURDATE() and c.status = 1 and c.customer_type = 1 and c.brand_id IS NULL ";
      ...
      $order = "c_date ASC";
      $data['ranking_platform'] = $this->ranking_model->getProductranking($where_prodcut,$order);

      //echo $this->db->last_query(); 
  	
     // print_ex($data['ranking_platform']);
      
      
   
    //print_ex($brand2_details);
  ...
  { 
   ...
   // print_ex($segs);     
    ...
    $getbrand = $this->uri->segment(2);
    $getcourse = $this->input->get('course');
   ...
   // print_ex($brand_id);
    ...
    $where_brand = 'rbw_status = 1';

     if(!empty($getcourse)){
      $where_brand .=" and pr.class_id = ".$getcourse." ";
      
    }

    $data['brand_records'] = $this->ranking_model->getbrand_ranking($where_brand);

    foreach($data['brand_records'] as $rec){

       $lastMonth =  $this->ranking_model->get_lastmonthranking($rec->brand_id,$getcourse);
       if(!empty($lastMonth)){
        $rec->last_month = $lastMonth['0']->rank;
       }else{
        $rec->last_month = "-";
       }
       

       $currentMonth =  $this->ranking_model->get_currentmonthranking($rec->brand_id,$getcourse);

       if(!empty($currentMonth)){
        $rec->current_month = $currentMonth['0']->rank;
       }else{
        $rec->current_month = "-";
       }
    ...
    //print_ex($data['blog_records']);
      $this->load->view('common/header',$data);
      $this->load->view('ranking/brand_ranking',$data);
      $this->load->view('common/footer');
  }

  function faculty_ranking($id = '')
  { 
    //die('dsds');
   ...
   // print_ex($brand_id);
    ...
    //print_ex($data['blog_records']);
      $this->load->view('common/header',$data);
      $this->load->view('ranking/faculty_ranking',$data);
      $this->load->view('common/footer');
  }

  function course_ranking($id = '')
  { 
    //die('dsds');
   ...
   // print_ex($data); 
    ...
    //print_ex($data['blog_records']);
      $this->load->view('common/header',$data);
      $this->load->view('ranking/course_ranking',$data);
      $this->load->view('common/footer');
  }

  function price_ranking($id = '')
  { 
    //die('dsds');
   ...
   // print_ex($brand_id);
    ...
    //print_ex($data['blog_records']);
      $this->load->view('common/header',$data);
      $this->load->view('ranking/price_ranking',$data);
      $this->load->view('common/footer');
  }
  ...
  {  
      ...
      $ranking = $this->jewelry_model->getProductrankingLimit($where,10,0);

      $where = "product_id = ".$jewelry_id." and status='active' ";
      $ranking_count = $this->jewelry_model->getProductrankingCount($where);      

      //$where = "product_id = ".$jewelry_id."" ;
      $ranking_sum = $this->jewelry_model->getProductrankingSum($where);

      $ranking_rating = @round($ranking_sum['0']->ranking_sum / $ranking_count['0']->ranking_count);
      ...
      $data= array(
          ...
          'ranking_count'=>$ranking_count['0']->ranking_count,
  ...
  function ranking_jewelry()
  { 
      $this->load->view('common/header');
      $this->load->view('jewelry/ranking_jewelry');
      $this->load->view('common/footer');
  } 
  ...
  {  
      ...
      $ranking_jewelry=array();

      $ranking_jewelry=$this->session->userdata('ranking_jewelry');
      $ranking_jewelry=@implode("','", $ranking_jewelry);

      $where = "product_id IN ('".$ranking_jewelry."')";      
      $attribute = $this->jewelry_model->jewelry_option($where);

      $records = $this->jewelry_model->jewelry_ranking($where);
      foreach ($records as $row) 
      {
        $img=$this->list_image($row->NM_FOLDER_NAME,$row->product_id);
        $row->image_show = @$img['0'];
  ...
  function add_ranking()
  {
      if(!$this->input->is_ajax_request()){ exit('No direct script access allowed'); }
      $ranking_jewelry=array();
      $message='Added To ranking';
      $status='0';
      if($this->session->userdata('ranking_jewelry'))
      {
        $ranking_jewelry=$this->session->userdata('ranking_jewelry');
      }    
      $jewelry_id=$this->input->get('jewelry_id');

      if (($key = array_search($jewelry_id, $ranking_jewelry)) !== false) {
        unset($ranking_jewelry[$key]);
        $message='Removed From ranking';
      }
      else if(count(array_unique($ranking_jewelry))<10){        
        $ranking_jewelry[]= $jewelry_id;       
      }
      else{
        $message='ranking List got Full. Please Remove Some Item.(Max:10)'; 
      }

      $this->session->set_userdata('ranking_jewelry',array_unique($ranking_jewelry));
      echo json_encode(array('message'=>$message,'status'=>$status));
  }
  function remove_ranking()
  {
      if(!$this->input->is_ajax_request()){ exit('No direct script access allowed'); }
      $id=$this->input->get('id');
      $ranking_jewelry=$this->session->userdata('ranking_jewelry');

      if (($key = array_search($id, $ranking_jewelry)) !== false) {
        unset($ranking_jewelry[$key]);
      }

      $this->session->set_userdata('ranking_jewelry',array_unique(array_filter($ranking_jewelry)));
      echo json_encode('ok');
  }

  function product_ranking_submit()
  {
      ...
      $ranking_rating=$this->input->post('ranking_rating');
      $ranking_title=$this->input->post('ranking_title');
      $ranking_content=$this->input->post('ranking_content');

      $where=array(
        'product_id'=>$product_id,
        'user_id'=>$this->session->userdata('user_id')
      );
      $details=$this->common_model->selectWhere('tbl_product_ranking',$where);
      //print_ex($details);
      if(!empty($details)){
        $data=array(
          'product_ranking_title'=>$ranking_title,
          'product_ranking'=>$ranking_content,
          'product_rating'=>$ranking_rating['0'],
          'product_ranking_added'=>date('Y-m-d'),
        );        
        $this->common_model->updateData('tbl_product_ranking',$data,$where);
      }else{
        $data=array(
          'product_id'=>$product_id,
          'user_id'=>$this->session->userdata('user_id'),
        ...
        $this->common_model->insertData('tbl_product_ranking',$data);
      }
      redirect($_SERVER['HTTP_REFERER']);
  }
  function product_ranking()
  {    
      $segment_2=$this->uri->segment(2);
      $image_array=array(); 
      ...
      //print_ex($data);
      $data['page_head'] = 'Jewelry ranking';

      $this->load->view('common/header',$data);
      $this->load->view('jewelry/jewelry_ranking',$data);
      $this->load->view('common/footer');
  }
  function load_product_ranking()
  {
      if(!$this->input->is_ajax_request()){ exit('No direct script access allowed'); }
      ...
      $records_count = $this->jewelry_model->getProductrankingCount($where);      
      $data['records_count'] =@$records_count['0']->ranking_count;  
 
      $per_page= ($per_page) ? $per_page : 10 ;
      $config['base_url'] = base_url().'load-product-ranking';
      $config['total_rows'] = $data['records_count'];
      $config['per_page'] = $per_page;
      $config['page_query_string'] = true;
      $config['query_string_segment'] = 'page';
      ...
      $records = $this->jewelry_model->getProductrankingLimit($where,$per_page,$page);
  ...
  function ranking_submit() {
       ...
       // print_ex($_REQUEST);
        $this->form_validation->set_rules('rankingId', 'rankingId', 'trim|required');
       ...
       // echo $this->db->last_query(); 
            ...
            $id = $this->common_model->insertData('tbl_product_ranking_booking', $data);


             //$userDetails=$this->common_model->selectOne('tbl_customer','customer_id',$rankingId);   
             $userDetails=$this->common_model->selectOne('tbl_customer','customer_id',$userId);  
             $order = "c_date ASC";
             $where = "c_id = ".$rankingId."";
            $ranking_list = $this->ranking_model->getProductrankingLimit($where,1,0);
       //echo $this->db->last_query();
      // print_ex($ranking_list);      
              ...
              $this->email->from(SITE_EMAIL,SITE_NAME);      
              $this->email->to(SITE_EMAIL);
              $this->email->subject("ranking Booking");      
