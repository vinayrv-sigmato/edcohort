...
class Auto_excel extends CI_Controller {
	...
	function check_header($header)
	{
		...
		$this->db->from('tbl_accepted_header AH');
		$this->db->join('tbl_header H', 'AH.HEADER_ID = H.HEADER_ID','right');  
	...
	function check_diamond($CD_VENDOR_ID,$SN_SERIALNO)
	{	
		$where .= "WHERE CD_VENDOR_ID = '".$CD_VENDOR_ID."' "; 
		$where .= " AND SN_SERIALNO = '".$SN_SERIALNO."' "; 
		$select='CD_INVENTORY_ID,CD_VENDOR_ID,SN_SERIALNO';
    ...
    {   
        //print_ex($data);
        if($data['SN_SERIALNO']!="" && $data['SN_SHAPE']!="" && $data['QT_WEIGHT']!="" && $data['SN_COLOR']!="" && $data['SN_GRADE']!="" 
            && $data['SN_LAB']!="" && $data['FL_RAPNET']!="" && $data['FL_RAPNET_DISCOUNT']!="")
        {
            return true;
        } 
        else
	...
	{
		exit; 
		$file='../ftp_upload/admin_jewelstree/product/excel/halo24-1-20.xlsx';
		...
		if(file_exists($file))
		{
			...
			{	
				if($ekey>0) {
				    ...
				    $product_sort = $datax['product_sort'];
				    $product_description = $datax['product_description'];
